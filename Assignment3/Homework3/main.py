...
if __name__ == '__main__':
	if len(sys.argv) != 5:
		print 'Usage: python *.py [train file] [test file] [output] [lang]'
		sys.exit(0)
	lang = sys.argv[4]
	window = 3
	xmldoc = minidom.parse(sys.argv[1])
	lex_list = xmldoc.getElementsByTagName('lexelt')
	(clf_map, voca_all_map, sens_all_map) = train_all(lex_list, window, 'svm', 15, lang)
	test_all_output(clf_map, voca_all_map, sens_all_map, sys.argv[2], sys.argv[3], lang)
